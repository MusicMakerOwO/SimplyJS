...
import { DiscordApplication, DiscordChannel, DiscordMember, DiscordSticker } from "../Types/DiscordAPITypes.js";
...
export class Message extends APIClientStructure<DiscordMessage> {
	 ...
	 * and not otherwise parsed into a {@link Member}. Partial because Discord omits the `user`
	 * field here - it would duplicate the message's `author`.
	 */
	member?: Partial<DiscordMember> | null
	...
	constructor(client: Client, data: DiscordMessage & { guild_id?: string | null; member?: Partial<DiscordMember> | null }) {
		super(client);
		this.patch(data);
	}

	patch(data: DiscordMessage & { guild_id?: string | null; member?: Partial<DiscordMember> | null }): void {
		this.id = data.id;
