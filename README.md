...
- Gateway event coverage is partial. Dispatch handlers exist for guilds, channels, threads, members, roles, messages, reactions, emojis, stickers, invites, presences, and typing indicators, but events like `VoiceStateUpdate` and thread membership (`THREAD_LIST_SYNC`, `THREAD_MEMBER_UPDATE`, `THREAD_MEMBERS_UPDATE`) are not yet handled.
