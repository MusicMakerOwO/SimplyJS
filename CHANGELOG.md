...
#### Components v2
- `ComponentBuilder` (`src/Builders/ComponentBuilder.ts`) — abstract base for component builders, carrying the optional `id` every component shares and a chainable `setId()` that rejects non-integers and values outside 32 bits. `id` is what lets a specific component be targeted when editing a message. Every v2 builder extends it, and `ActionRowBuilder` is the first v1 builder to — its `from()` now takes a full `ActionRow` as well as a bare components array, so a row inside a container keeps its `id` across a round-trip. The button and select builders still have no `id` support; `Button` needs a type change first, since it is the one component built on `BaseButton` rather than `BaseComponent` and so carries `custom_id` but no `id`
...
- `SectionBuilder` (`src/Builders/SectionBuilder.ts`) — 1-3 `TextDisplay`s with an accessory alongside them. The bounds and a missing accessory are `validate()` cases, but the accessory's *type* is rejected eagerly in `setAccessory()`, since a component that is neither a button nor a thumbnail is a structural mistake rather than an unfinished one
...
- `ContainerBuilder` (`src/Builders/ContainerBuilder.ts`) — a bordered, optionally accented group holding any of the other components, with an optional nullable `accent_color` and `spoiler`. A nested container is rejected with its own error message rather than the generic wrong-type one, since it is the mistake most worth naming. `setAccentColor(null)` clears the color, which is distinct from leaving it unset
